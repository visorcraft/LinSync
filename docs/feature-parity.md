...
| --- | --- | --- |
| File compare | Side-by-side text comparison, inline highlights, navigation, merge-copy actions, save safety, encoding and EOL handling | Complete |
| Folder compare | Recursive and non-recursive comparison, selectable methods, filters, row states, copy/delete/rename plans, refresh and cancel behavior — plus windowed paging, sort/filter/group-by, multi-type filter | Complete |
| Three-way merge | Base/left/right comparison, conflict markers, conflict navigation, result save workflow | Complete |
...
| CLI | Stable commands, shell completions, man page, documented exit-code contract, launch/open/reveal helpers | Complete |
| Settings and sessions | XDG JSON stores, recent paths/sessions, project files, migration, import/export, reset, and concurrency safety | Complete |
| Plugins | JSON-over-stdio helpers, manifest validation, bounded execution, discovery, classes, settings UI, sandbox policy | Complete |
| GUI shell | Qt 6 / Kirigami sidebar with Compare, Image / Webpage / Document Compare workspaces, Sessions, Filters, Plugins, Settings, About — plus Credits and Licenses reached from About | Complete |
