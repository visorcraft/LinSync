...
| Archive-as-folder compare | complete (`compare_archives_with_unpacker`, `…_recursive`, `extract_archive_member`) | complete (`archive [--unpacker PLUGIN_ID]`) | n/a | n/a | n/a | partial | partial (`known-limitations-1.0.md`) | Routes each archive through an unpacker / folder-virtualizer plugin into a virtual tree (sandboxed), with nested-archive recursion and per-member extraction. The CLI's built-in `tar` / `unzip` fast path for common formats still shells out directly (unsandboxed) and is used when no plugin is required. |
