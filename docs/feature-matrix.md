...
| Table compare (CSV/TSV) | complete (`linsync-core::table`) | complete (`table`, `compare --type table`) | complete (`/compare`, `/compare/table/window`) | partial (via `compare_paths`) | complete (Compare page grid with headers, per-cell state, inline diffs, and windowing) | complete | complete | Bridge query params `key_columns`, `ignore_columns`, `numeric_tolerance`, `ignore_row_order` are parsed and applied. Large tables are windowed through `/compare/table/window`. |
...
| Document compare | complete (`linsync-core::document`) | complete (`compare --type document`, `--save-result`) | complete (`/compare/document`, text/ocr_text/rendered sub-modes, rendered-page URIs) | n/a | complete (Document Compare page: text, OCR text, and rendered page navigator) | complete | partial (`known-limitations-1.0.md`, `document-compare-implementation.md`) | OCR mode requests per-word positions (`want_positions`) and surfaces them as data; rendered mode emits `rendered_pages` with `file://` URIs and a per-page thumbnail/image viewer. Saved result round-trips through `report --from-json`. Carve-out: no zoom-aware visual overlay of OCR boxes on a rendered page. |
| Webpage compare — source / text / tree | complete (`linsync-core::webpage`) | complete (`webpage --sub-mode html|text|tree`) | complete (`/compare/webpage`, all sub-modes; cache flush via `/compare/webpage/clear-cache`) | n/a | complete (Webpage Compare page, diff rows + tree view) | partial | partial (`known-limitations-1.0.md`) | `/compare/webpage` requires `confirmed=1` from the QML consent dialog; the bridge still ignores profile `confirmed_by_user` so persisted profiles cannot bypass the dialog. Diff rows and resource-tree entries surfaced in QML. |
...
| Archive-as-folder compare | complete (`compare_archives_with_unpacker`, `…_recursive`, `extract_archive_member`, `archive::compare_builtin_archives`) | complete (`archive [--unpacker PLUGIN_ID]`) | complete (`/compare` auto-routes archive pairs; built-in extractor fallback) | n/a | complete (Compare page folder view for archive pairs, mode selector includes Archive) | partial | partial (`known-limitations-1.0.md`) | Plugin-based path is sandboxed; built-in `tar`/`unzip` fast path runs under `linsync-sandbox` and is exposed in the GUI when no plugin applies. |
| Archive member editing | complete (`archive_write::extract_member_for_edit`, `commit_member_edit`) | n/a | complete (`/archive/member/edit`, `/archive/member/commit`, `/archive/member/discard`) | n/a | complete (context menu + commit/discard bar in folder view) | complete | complete (`archive-write-safety-design.md`) | Zip, tar (plain and compressed), and 7z. Atomic replace, `.bak` backup, sandboxed helpers, TOCTOU fingerprinting. 7z requires the 7z/p7zip binary. |
