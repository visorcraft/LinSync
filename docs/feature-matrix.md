...
| Folder compare | complete (`linsync-core::folder`) | complete (`folders`) | complete (`/compare`, `/folder/op/plan`, `/folder/op/execute`) | partial (`compare_paths`) | partial (Compare page, capped row count) | complete | complete | `folder_query_bridge_response` uses `resolve_folder_options_for_request` so the active profile / walk options are honored. |
...
| Image compare | complete (`linsync-core::image`) | complete (`compare --type image`, `--image-frames first\|all`, `--save-result`) | partial (`/compare/image`, `?frames=`) | n/a | partial (Image Compare page) | complete | complete (`user-guide.md`, `known-limitations-1.0.md`, `image-compare-design.md`) | Real overlay PNG, padded dimension-mismatch canvas, diff-region navigation, format UI, and save-overlay endpoint are present. Now decodes GIF/HDR/EXR, compares animated frames (`FirstFrame` default / `AllFrames`), records color-type metadata, and round-trips a saved result through `report --from-json`. Remaining gap: animation-timeline semantics and ICC/HDR fidelity (tone-mapped to RGBA8). |
...
| Sessions / recent paths | complete | partial (`launch`) | complete (`/session`, `/sessions/recent`, `/sessions/reopen`, `/sessions/delete`, `/sessions/rename`) | partial (`session_json`, etc.) | partial (Sessions page) | partial | partial | Bridge endpoints for delete/rename are wired; QML UI still read-only. |
| Reports & patches | complete (`linsync-core::report`, `patch`) | complete (`patch`, `report`, `compare --save-result`, `report --from-json`) | n/a | n/a | n/a | complete | complete | `--save-result` / `report --from-json` round-trips text, folder, table, binary, **image, and document** results to HTML. No GUI export yet. |
...
| --- | --- | --- | --- |
| Compare | `Main.qml` | text / folder via `/compare` and `compare_paths`; Stop button cancels via `/cancel?id=X` | partial |
...
| Document Compare | `DocumentComparePage.qml` | `/compare/document` | partial — left/right extracted-text panes surfaced; no rendered-page overlay |
