...
| Archive member editing (zip) | complete (`archive_write::extract_member_for_edit`, `commit_member_edit`) | n/a | complete (`/archive/member/edit`, `/archive/member/commit`, `/archive/member/discard`) | n/a | complete (context menu + commit/discard bar in folder view) | complete | complete (`archive-write-safety-design.md`) | Zip only; tar/7z read-only. Atomic replace, `.bak` backup, sandboxed helpers, TOCTOU fingerprinting. |
