...
| Image compare | complete (`linsync-core::image`) | complete (`compare --type image`) | partial (`/compare/image`) | n/a | partial (Image Compare page) | complete | partial (`user-guide.md`, `known-limitations-1.0.md`) | Overlay PNG is a transparent placeholder; Save-Overlay button explicitly disabled; dimension mismatch is a hard error. |
...
| Webpage compare — rendered / screenshot | **stub** (`linsync-webengine` returns `NotImplemented`) | partial (`webpage --sub-mode rendered|screenshot`, returns error) | stub | n/a | partial (controls present, error reported) | minimal | partial | Behind `web-engine` Cargo feature. |
...
| Image Compare | `ImageComparePage.qml` | `/compare/image` (overlay is placeholder; Save-Overlay button disabled) | partial |
